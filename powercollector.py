...
parser = argparse.ArgumentParser(prog='powercollector',
                                 description='Connect to an HMC to collect configuration and events. A full '
                                             'collection will also connect to each Running LPAR to collect OS-level '
                                             'data. To only collect HMC-level data, use the --hmconly flag. To '
                                             'only collect the OS-level data, provide a previously generated JSON '
                                             'file with the --input parameter.')
...
print('powercollector version 1.0.5')
...
# per https://stackoverflow.com/questions/404744/determining-application-path-in-a-python-exe-generated-by-pyinstaller
...
logger.add(output_dir + '\\' + 'powercollector-Main-log_{time:YYYY-MM-DD}.log',
           ...
           level="INFO")
logger.info('powercollector version 1.0.5')
...
if not args.input:
    ...
    if check_host(args.hmc):
        ...
        # The PEP manual doesn't like handling broad exceptions but since we already handle the exceptions in the module
        # what is wrong with just propagating all the way to main and exiting here?
        ...
        for system in managed_systems:
            ...
            # temp_ecnumber_primary:temp_level_primary:temp_ecnumber_secondary:temp_level_secondary:perm_ecnumber_primary
            # :perm_level_primary:perm_ecnumber_secondary:perm_level_secondary
            response = hmc_ssh.execute_command('lslic -t sys -m ' + system.name + ' -F temp_ecnumber_primary:' +
                                               ...
                                               'temp_ecnumber_secondary:temp_level_secondary:perm_ecnumber_secondary:' +
                                               'perm_level_secondary', 30)
