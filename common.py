...
class EnclosureTopology(Jsonizable):
    ...
    def __init__(
        ...
        json_in=None,
        enclosure=None,
        leading_hub_port=None,
        trailing_hub_port=None,
    ):
...
def save_hmc_data(hmc_src, hmc, output_dir):
    ...
    output_file = (
        output_dir + "\\" + hmc_src + "-SystemsManagedByHMC-" + hmc.hostname + ".json"
    )
...
def save_os_level_data_for_sys(
    ...
    for system in managed_systems:
        ...
        logger.info(
            "LPAR OS-level collection for System: " + system.name + " completed."
        )
    if non_collected_lpars:
        print_red(
            "Unable to collect OS-level data for some LPARs, please"
            + " run oscollector manually on each one. Check the log file for the LPAR list."
        )
        logger.error(
            "Unable to collect OS-level data for the following LPARs, please"
            + " run oscollector manually on each one."
        )
        for lpar in non_collected_lpars:
            logger.info(
                f"System LPAR: {lpar.name} rmc_ip: {lpar.rmc_ip} state: {lpar.state}"
            )
        with open(output_dir + "\\" + "NonCollectedLPARList.json", "w+") as file:
            ...
            logger.info(
                "List of LPARs that require manual collection written to file: "
                + output_dir
                + "\\"
...
def save_lpar_os_data(
    ...
    if "Running" not in lpar.state:
        ...
        print_red(
            "LPAR: " + lpar.name + " LPAR must be running to be collected. Please "
            "run oscollector manually if needed."
        )
        logger.error(
            "LPAR: " + lpar.name + " must be running to be collected. Please "
            "run oscollector manually if needed."
        )
        return False
    ...
    if lpar.rmc_ip == "":
        ...
        print_red(
            "LPAR: "
            + lpar.name
            + " is running but doesn't have an RMC IP address. Please "
    ...
    if not check_host(lpar.rmc_ip):
        ...
        print_red(
            "Error during connection to LPAR: "
            + lpar.name
            + ", please check log file and run "
    ...
    for attempt in range(5):
        print(
            "Please input username and password or press enter to use the proposed value."
        )
        # Ask for input, if the input is empty, use the current value
        username = (
            input("Username for LPAR: " + lpar.name + " (" + username + "): ")
            or username
        )
        password = (
            input("Password for user " + username + " (" + password + "): ") or password
        )
        try:
            lpar_ssh = RemoteClient(
                host=lpar.rmc_ip, user=username, password=password, remote_path="."
            )
            ...
            if attempt == 4:
                return False
            logger.info(
                "Authentication error. Retrying connection with LPAR:"
                + lpar.name
                + ", attempt "
                + str(attempt + 2)
                + " of 5"
        ...
        # Fun Fact: Try Except blocks also have an else condition, it's triggered when it exits cleanly.
            ...
            try:
                ...
                if not lpar.env:
                    ...
                    response, _ = lpar_ssh.execute_command(
                        "uname -s", 30, want_errors=True
                    )
                    if not response:
                        response, _ = lpar_ssh.execute_command(
                            "lsdev | grep vios0", 30, vios=True, want_errors=True
                        )
                        if response:
                ...
                response = lpar_ssh.execute_command(
                    "ksh ./" + oscollector, 900, vios=set_vios
                )
                ...
                if not old_name:
                    print_red(
                        "Error encountered during transfer or execution of script on LPAR: "
                        + lpar.name
                        + ", please check log file and run oscollector manually on the LPAR."
                    )
                    logger.error(
                ...
                lpar_ssh.execute_command(
                    "mv " + old_name + " " + output_file + ".tar",
                    60,
                    want_errors=True,
                    vios=set_vios,
                )
                ...
                try:
                    ...
                    lpar_ssh.execute_command(
                        "rm " + old_name + "-config.txt",
                        30,
                        want_errors=True,
                        vios=set_vios,
                    )
                    lpar_ssh.execute_command(
                        "rm " + old_name + "-error.txt",
                        30,
                        want_errors=True,
                        vios=set_vios,
                    )
                    lpar_ssh.execute_command(
                        "rm " + old_name + "-lsgcl.txt",
                        30,
                        want_errors=True,
                        vios=set_vios,
                    )
                    lpar_ssh.execute_command(
                        "rm " + oscollector, 60, want_errors=True, vios=set_vios
                    )
                    lpar_ssh.execute_command(
                        "rm " + old_name + ".tar", 30, want_errors=True, vios=set_vios
                    )
                    lpar_ssh.execute_command(
                        "rm " + output_file + ".tar",
                        30,
                    ...
                    print_red(
                        "Error encountered during cleanup on LPAR: "
                        + lpar.name
                    ...
                    if __debug__:
                        logger.exception(e)
                    logger.error(
                        "Error encountered during during cleanup on LPAR: "
                        + lpar.name
    ...
    # Another Fun Fact: For loops ALSO have else conditions, this one is triggered on loop reaching the end.
        ...
        print_red(
            "Error during connection to LPAR: " + lpar.name + ", please check log file."
        )
        logger.error(
            "Error during connection to LPAR: "
            + lpar.name
...
def run_hmc_scan(hmc_scan_path, base_dir, hmc, user, password, output_path):
    ...
    if java:
        hmc_scanner_command = (
            ...
            + '"'
            + " -html"
            + f" -log {output_path}\\hmcscanner.log"
        )
